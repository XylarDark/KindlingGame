...
- **Fix:** BIP handler uses `presentInstallCoach({ force: true })`; audience includes `any-pointer: coarse`, `maxTouchPoints > 1`, and Android/iOS UA; adaptive RenderBudget (phones seed mid @ 0.55×, PostFX off); **desktop / IDE panes use `containStage(..., "contain")`** so tall Cursor browser views letterbox instead of side-cropping (`left: -229` style zoom).
