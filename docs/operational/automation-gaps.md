...
## Android Emulator validates PWA install, not FPS

- **Date:** 2026-09-09
- **Feature / area:** proving mobile smoothness and PWA/service-worker behaviour after RenderBudget / idle-SW work.
- **What is needed:** one commandable path on Windows that exercises Install / standalone / `navigator.serviceWorker.controller` / idle activate+reload the way a phone does.
- **Why automation fails (for FPS):** AVD frame times ride the **host GPU or software GL**, not a mid-phone Adreno/Mali. Emulator `actualFps` can look fine while a real device stutters (or the reverse). Treating the emulator as the RenderBudget ship gate would rubber-stamp the wrong number.
- **Interim:** `npm run emu:pwa` + [guides/android-emulator.md](../guides/android-emulator.md) for **install + SW controller + idle activate**. FPS acceptance stays on a **physical phone** or a **lane PostFX on/off A/B** (`scripts/agent-shot.ts`). iOS Simulator stays out of scope on Windows.
- **Suggested follow-up:** none for FPS-on-emulator; keep the split explicit in any future mobile QA doc.
