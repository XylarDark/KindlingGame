...
- **Why automation fails:** the only hook point is a Cursor editor hook, and it cannot be made to fail closed. Cursor discards hook output above roughly 1.4KB and any transport flake is indistinguishable from a clean scan, so a blocking posture denies ordinary file reads at random. Fail-open is the usable posture, and a fail-open scanner does not prevent anything — it blocks a secret when its verdict happens to arrive and allows it otherwise. That is an audit log wearing the costume of a control, so the hook was removed rather than kept for reassurance.
...
- **Suggested follow-up:** none while both the hook payload limit and the notification cost stand. Be clear about the residual risk rather than reassured by this entry: **nothing in this repo scans for secrets.** A secret pasted into a working file can enter an agent transcript, and a committed one reaches the remote unchallenged. What makes that tolerable is the absence of anything to leak, which is a property of the design and would stop being true the moment this game grows a backend. The defects that killed the hook are kept in [KNOWN_ERRORS.md](../KNOWN_ERRORS.md) as the specification for anyone reintroducing one.
